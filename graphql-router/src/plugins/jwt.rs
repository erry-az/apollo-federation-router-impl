...
// Custom Claims for forward header
#[derive(Clone, Debug, Serialize, Deserialize)]
struct RouterCustomClaims {
    user_id: String,
    roles: Vec<String>,
}
    ...
    fn try_initialize_hmac(configuration: &Conf, key: String) -> Option<JwtHmac> {
        ...
        if let Some(algorithm) = hmac_algorithm {
            let verifier = match algorithm {
                HMACAlgorithm::HS256 => {
                    HMACVerifier::HS256(HS256Key::from_bytes(base64::decode(key).ok()?.as_ref()))
                }
                HMACAlgorithm::HS384 => {
                    HMACVerifier::HS384(HS384Key::from_bytes(base64::decode(key).ok()?.as_ref()))
                }
                HMACAlgorithm::HS512 => {
                    HMACVerifier::HS512(HS512Key::from_bytes(base64::decode(key).ok()?.as_ref()))
                }
            };
            hmac = Some(JwtHmac {
                algorithm,
...
impl Plugin for JwtAuth {
    ...
    async fn new(init: PluginInit<Self::Config>) -> Result<Self, BoxError> {
        ...
        let hmac = JwtAuth::try_initialize_hmac(&configuration, key.clone());
        ...
        ServiceBuilder::new()
            .checkpoint(move |req: supergraph::Request| {
                ...
                if let Some(verifier) = &hmac_verifier {
                    match verifier.verify_token::<RouterCustomClaims>(
                        jwt,
                        Some(options),
                    ) {
                        Ok(claims) => {
                            ...
                            let claim_user_id = claims.custom.user_id.clone();
                            let claim_roles = claims.custom.roles.clone().join(",");

                            tracing::debug!("Show User ID : {}", claim_user_id.clone());
                            tracing::debug!("Show Roles : {}", claim_roles.clone());

                            // We are happy with this JWT, on we go...
                            // Let's put our claims in the context and then continue
                            match req.context.insert("router-user-id", claim_user_id) {
                                Ok(_v) => {
                                    if let Err(e) = req.context.insert("router-roles", claim_roles) {
                                        tracing::warn!("fail parse context roles {}", e);
                                    };
                                    Ok(ControlFlow::Continue(req))
                                },
                                Err(err) => {
                                    return failure_message(req.context,
                                                           format!("couldn't store user-id in context: {}", err),
...
register_plugin!("graphql_router", "jwt", JwtAuth);
    ...
    async fn test_hmac_jwtauth_accepts_valid_tokens() {
        ...
        mock_service
            ...
            .returning(move |req: supergraph::Request| {
                ...
                let router_user_id: String = req
                    .context
                    .get::<String, String>("router-user-id".to_string())
                    .expect("claims are present") // Result
                    .expect("really, they are present"); // Option

                assert_eq!(router_user_id, "123".to_string());

                let router_user_roles: String = req
                    .context
                    .get::<String, String>("router-roles".to_string())
                    .expect("claims are present") // Result
                    .expect("really, they are present"); // Option

                assert_eq!(router_user_roles, "yz".to_string());
        ...
        let verifier = HS256Key::from_bytes(base64::decode(key).unwrap().as_ref());
        let mut audiences = HashSet::new();
        audiences.insert("audience 1".to_string());
        audiences.insert("audience 2".to_string());
        let claims = Claims::with_custom_claims(
            RouterCustomClaims{ user_id: "123".to_string(), roles: vec!["yz".to_string()] },
